...
对 **217 个真实站点、五轮完整跑动**（累计 1087 站次）跑核心链路
...
| 第 5 轮（最终） | **5.0%** | **63%** | **4 / 217** |

「哑 ref」= 快照里没有任何可读文字的元素，AI 无从判断点它会发生什么。
「硬失败」= 导航或快照调用本身失败。后三轮指标在窄带内波动，属正常
（网络状况、站点可用性、临时风控每轮都不同），已达稳定。

**衡量「稳定」的标准不是失败归零**——反爬 403、证书错误、第三方扩展干扰、
重 JS 站点主线程饱和都是客观存在，不可能消除。真正的标准是：
**每一个失败都能被 AI 读懂并正确处置**。第 5 轮的 4 例失败全部满足：

| 站点 | 错误码 | 信息是否可操作 |
|---|---|---|
| 淘宝 | `NO_TAB` | ✅ 「标签页已被关闭——标签管理类扩展会批量收纳；用 list_tabs 查存活 tab 或新标签页重开」 |
| 知网 | `FORBIDDEN` | ✅ 「Chrome 拒绝附着——多为安全拦截页（证书/隐私错误），此类页面无法检查」 |
| eBay | `TIMEOUT` | ✅ 「主线程饱和（重 JS/仍在加载）——先 wait_for network_idle 再重试」 |
| 和风天气 | `TIMEOUT` | ✅ 同上 |
